...
void killhand(int sig){
    ...
    if (server){
        ...
        if (msgctl(msg_rep,IPC_RMID,NULL) == -1){
            perror("Cannot remove reply queue");
            exit(1);
        }
    }
    exit(0);
...
int main(int argc, char *argv[])
{
    ...
    rep_key = ftok(tmpfname, MSG_REPLY);
    if (req_key == -1 || rep_key == -1){
        perror("Key generation error");
        return -1;
    }
    msg_req = msgget(req_key, perm | IPC_CREAT);
    msg_rep = msgget(rep_key, perm | IPC_CREAT);
    if (msg_req == -1 || msg_rep == -1) {
        perror("Create msg queue error");
        return -1;
    }
    printf("PID %d: Request - %d; reply - %d\n", getpid(), msg_req, msg_rep);
    int clen = 0;
    int maxlen = 128;
    struct mymsg mbuf;
    if (server){
        while(1){
            clen = msgrcv(msg_req, &mbuf, maxlen, 0, 0);
            if(clen == -1){
                perror("Message receive error");
            }
            else{
                sprintf(mbuf.mtext,"Server receive message from %d", mbuf.mtype);
                if(msgsnd(msg_rep, &mbuf, strlen(mbuf.mtext)+1, 0) == -1){
        ...
        do{
            ...
            if (msgsnd(msg_req, &mbuf,strlen(mbuf.mtext)+1, 0) == -1){
                perror("Client send message error");
                return -1;
            }

            while (rcnt != -1){
                rcnt = msgrcv(msg_rep,&mbuf,maxlen,getpid(),IPC_NOWAIT);
                if (rcnt != -1)
                    printf("PID %d: %s\n", getpid(), mbuf.mtext);
            }
            rcnt = 0;
